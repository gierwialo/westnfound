...
class ZrzutkaService:
    """Ile ludzie wpłacili na zbiórkę — jedna publiczna liczba i nic więcej.

    Pyta wyłącznie `GET /api/v1/public/whipRounds/{id}`. Końcówek `/payments`
    i `/moneyBoxes/.../payments` NIE dotykamy świadomie: niosą dane osób, które
    wpłaciły, a do paska postępu potrzebna jest jedna suma. Zasada z
    docs/Product/Feature-Support.md: nie pobierać danych wpłacających ani salda
    do wypłaty.

    Token żyje wyłącznie tutaj, po stronie serwera. Do aplikacji i do frontendu
    jedzie już tylko gotowa liczba — to twardy warunek tej funkcji, nie wygoda.

    Cache dwustopniowy, jak przy kanale kalendarza: świeża kopia na godzinę,
    ostatnia dobra na tydzień. Zbiórka, której kwota jest o godzinę stara, to
    pasek postępu, którego nikt nie zauważy; zbiórka bez kwoty to pasek, który
    znika ludziom z oczu przy pierwszej czkawce cudzego serwera.
    """

    BASE_URL = 'https://zrzutka.pl/api/v1/public'
    FRESH_SECONDS = 60 * 60
    LAST_GOOD_SECONDS = 7 * 24 * 60 * 60
    TIMEOUT_SECONDS = 8

    def collected(self, whip_round: str, token: str) -> Optional[float]:
        """Zebrana kwota w złotych, albo None, gdy nigdy jej nie poznaliśmy."""
        if not whip_round or not token:
            return None

        fresh_key = f'zrzutka:fresh:{whip_round}'
        last_good_key = f'zrzutka:last-good:{whip_round}'

        cached = cache.get(fresh_key)
        if cached is not None:
            return cached

        amount = self._fetch(whip_round, token)
        if amount is not None:
            cache.set(fresh_key, amount, self.FRESH_SECONDS)
            cache.set(last_good_key, amount, self.LAST_GOOD_SECONDS)
            return amount

        last_good = cache.get(last_good_key)
        if last_good is not None:
            logger.warning('Zrzutka nieosiągalna, podaję ostatnią znaną kwotę')
            return last_good

        return None

    def _fetch(self, whip_round: str, token: str) -> Optional[float]:
        try:
            response = requests.get(
                f'{self.BASE_URL}/whipRounds/{whip_round}',
                headers={
            ...
            # Bez tokenu w komunikacie: log trafia na dysk i do `docker logs`.
            logger.error(f'Nie udało się zapytać Zrzutki: {exc}')
            return None

        if response.status_code != 200:
            logger.error(f'Zrzutka oddała HTTP {response.status_code}')
            return None

        try:
            data = response.json()
        except ValueError:
            logger.error('Zrzutka oddała coś, co nie jest JSON-em')
            return None

        amount = data.get('amountCollected')
        currency = data.get('currency')

        # Waluta sprawdzana, a nie zakładana: gdyby zbiórka kiedyś przeszła na
        # euro, podanie tej liczby jako złotych byłoby kwotą wyglądającą na
        # prawdziwą. Lepiej nie pokazać paska niż pokazać zły.
        if currency != 'PLN':
            logger.error(f'Zrzutka oddała walutę {currency}, spodziewano się PLN')
            return None

        if not isinstance(amount, (int, float)) or isinstance(amount, bool):
            logger.error('Zrzutka nie podała amountCollected jako liczby')
            return None
