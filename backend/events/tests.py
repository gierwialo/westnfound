...
@override_settings(
    ...
    SUPPORT_ANNUAL_COST_PLN=565.69,
    SUPPORT_HISTORICAL_COST_PLN=3331.39,
    ZRZUTKA_TOKEN='token-testowy',
    ZRZUTKA_WHIP_ROUND='whcpfa',
)
class SupportCollectedTests(TestCase):
    """Zebrana kwota: jedna publiczna liczba, token wyłącznie po stronie serwera."""
    ...
    def test_adds_the_collected_amount(self):
        with patch('events.services.requests.get') as get:
            get.return_value = self._answer(
                {'amountCollected': 225, 'currency': 'PLN'})
            body = self.client.get('/api/support-info/').json()

        self.assertEqual(body['collectedPln'], 225.0)

    def test_asks_only_the_whip_round_never_the_payments(self):
        """Dane wplacajacych nie moga nas w ogole interesowac.

        Zrzutka wystawia /payments i /moneyBoxes/.../payments. Do paska postepu
        potrzebna jest jedna suma, wiec zapytanie o cokolwiek wiecej byloby
        pobraniem cudzych danych bez powodu.
        """
        with patch('events.services.requests.get') as get:
            get.return_value = self._answer(
                {'amountCollected': 225, 'currency': 'PLN'})
            self.client.get('/api/support-info/')

        called = get.call_args[0][0]
        self.assertTrue(called.endswith('/whipRounds/whcpfa'))
        self.assertNotIn('payments', called)
        self.assertNotIn('moneyBoxes', called)

    def test_token_travels_in_the_header_and_not_to_the_reader(self):
        with patch('events.services.requests.get') as get:
            get.return_value = self._answer(
                {'amountCollected': 225, 'currency': 'PLN'})
            response = self.client.get('/api/support-info/')
        ...
        self.assertEqual(
            get.call_args[1]['headers']['Authorization'], 'Bearer token-testowy')
        self.assertNotIn('token-testowy', response.content.decode())

    def test_silence_from_zrzutka_drops_the_key_instead_of_sending_zero(self):
        """Zero znaczyloby "nikt nie wplacil", a nie "nie wiemy"."""
        with patch('events.services.requests.get',
                   side_effect=requests.RequestException('padlo')):
            body = self.client.get('/api/support-info/').json()

        self.assertNotIn('collectedPln', body)
        # Reszta odpowiedzi ma dzialac dalej - zbiorka jest dodatkiem.
        self.assertEqual(body['annualCostPln'], 565.69)

    def test_last_known_amount_survives_an_outage(self):
        with patch('events.services.requests.get') as get:
            get.return_value = self._answer(
                {'amountCollected': 225, 'currency': 'PLN'})
            self.client.get('/api/support-info/')

        # Swiezy cache wygasa, Zrzutka nie odpowiada - zostaje ostatnia dobra.
        cache.delete('zrzutka:fresh:whcpfa')
        with patch('events.services.requests.get',
                   side_effect=requests.RequestException('padlo')):
            body = self.client.get('/api/support-info/').json()
    ...
    def test_a_different_currency_is_not_shown_as_zlotys(self):
        with patch('events.services.requests.get') as get:
            get.return_value = self._answer(
                {'amountCollected': 225, 'currency': 'EUR'})
            body = self.client.get('/api/support-info/').json()

        self.assertNotIn('collectedPln', body)

    @override_settings(ZRZUTKA_TOKEN='')
    def test_without_a_token_we_simply_do_not_ask(self):
        with patch('events.services.requests.get') as get:
            body = self.client.get('/api/support-info/').json()
